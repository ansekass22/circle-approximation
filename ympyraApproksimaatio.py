...
import ympyraApprox as yApp
import circleApproxPlot as cap

#############################################################
# (C) Antti Kasslin
# 2022
#############################################################
# Ohjelma, jolla voidaan tehdä ympyrää mahdollisimman
# jäljitteleviä monikulmioita, eli ympyräapproksimaatioita.
#############################################################
def valintaToiminto():
    if var1.get() == 1:
...
def suoritus():
    PI = 3.14159265 #piin likiarvo "oikean" ympyrän mittoja varten
    sade = int(sadeValitsin.get())#käyttäjä antaa ympyrän säteen
    ...
    neljannesX,neljannesY,polkuX,polkuY = yApp.ympyraApprox(sade,tarkkuus)
    ...
    neljPintaAla = yApp.shoeLace(neljPlusOrigoX,neljPlusOrigoY)
    neljPiiri = yApp.piirinPituus(neljannesX,neljannesY)

    neljannesX, neljannesY = yApp.taydennaApprox(neljannesX,neljannesY)
    ...
    pa2.insert(0,str(PI*sade**2))
    piiri2.insert(0,str(PI*2*sade))

    if var1.get() == 1:
        cap.makePlot(sade,tarkkuus,neljannesX,neljannesY,polkuX,polkuY,var2,var3,var4)

# käyttöliittymän alustusta
juuri = Tk()
juuri.title("Ympyräapproksimaatio")
juuri.geometry('500x350')

# käyttöliittymän toimintojen sijoittelua
label1 = Label(juuri, text="Valitse säde")
sadeValitsin = Spinbox(juuri,from_=1,to=50,bd=5)
label1.grid(row=0,column=0,padx=5,pady=5)
sadeValitsin.grid(row=0,column=1,padx=5,pady=5)

label2 = Label(juuri, text="Valitse tarkkuus")
tarkkuusValitsin = Entry(juuri, justify='right', bd='5')
label2.grid(row=1,column=0,padx=5,pady=5)
tarkkuusValitsin.grid(row=1,column=1,padx=5,pady=5)
labelWarn = Label(juuri, text="") 
labelWarn.grid(row=1,column=2) #varoitusteksti, jos epäsopiva tarkkuus
...
valinta3 = Checkbutton(juuri,text='Näytä ympyrä              ', justify='left',variable=var3, onvalue=1, offvalue=0, state='disabled')
valinta4 = Checkbutton(juuri,text='Näytä algoritmipolku  ', justify='left',variable=var4, onvalue=1, offvalue=0, state='disabled')
valinta1.grid(row=2,column=0,columnspan=2,padx=5,pady=5)
valinta2.grid(row=3,column=1)
valinta3.grid(row=4,column=1)
valinta4.grid(row=5,column=1)

kaynnistys = Button(juuri,text="Suorita approksimaatio",command=suoritus, width=20, font=15)
kaynnistys.grid(row=6, column=0,columnspan=3,padx=5,pady=5)

label3 = Label(juuri, text="APPROKSIMAATIO")
label4 = Label(juuri, text="OIKEA YMPYRÄ")
label3.grid(row=7,column=1,padx=5,pady=5)
label4.grid(row=7,column=2,padx=5,pady=5)
label5 = Label(juuri, text="Pinta-ala:")
label6 = Label(juuri, text="Piiri:")
label5.grid(row=8,column=0,padx=5,pady=5)
label6.grid(row=9,column=0,padx=5,pady=5)
pa1 = Entry(juuri, justify='right', bd='5')
pa2 = Entry(juuri, justify='right', bd='5')
piiri1 = Entry(juuri, justify='right', bd='5')
piiri2 = Entry(juuri, justify='right', bd='5')
pa1.grid(row=8,column=1,padx=5,pady=5)
pa2.grid(row=8,column=2,padx=5,pady=5)
piiri1.grid(row=9,column=1,padx=5,pady=5)
piiri2.grid(row=9,column=2,padx=5,pady=5)
